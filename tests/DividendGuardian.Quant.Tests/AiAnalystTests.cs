...
public sealed class AiAnalystTests
{
    ...
    public async Task AnalyzeAsync_RetriesTransientThenSucceeds()
    {
        ...
        var handler = new StubHandler(_ =>
        {
            calls++;
            return calls == 1
                ? Response(HttpStatusCode.ServiceUnavailable, """{"error":"temporary"}""")
                : Response(HttpStatusCode.OK, ValidResponse());
        });

        var result = await CreateAnalyst(handler, 1).AnalyzeAsync(CreateRequest());
        Assert.Equal("ASII", result.Ticker);
    ...
    public async Task AnalyzeAsync_DoesNotRetryBadRequest()
    {
        ...
        {
            calls++;
            return Response(HttpStatusCode.BadRequest, """{"error":"bad request"}""");
        });

        await Assert.ThrowsAsync<HttpRequestException>(() => CreateAnalyst(handler, 3).AnalyzeAsync(CreateRequest()));
    ...
    public async Task AnalyzeAsync_RetriesTooManyRequestsUntilExhausted()
    {
        ...
        {
            calls++;
            return Response(HttpStatusCode.TooManyRequests, """{"error":"rate limited"}""");
        });

        await Assert.ThrowsAsync<HttpRequestException>(() => CreateAnalyst(handler, 2).AnalyzeAsync(CreateRequest()));
    ...
    {
        var handler = new StubHandler(_ => Response(HttpStatusCode.OK, """{"output_text":"{\"ticker\":\"TLKM\",\"verdict\":\"WATCH\",\"why_accumulate\":\"x\",\"why_not_accumulate\":\"y\",\"key_risks\":[],\"data_gaps\":[],\"invalidation_triggers\":[],\"data_quality\":\"READY\",\"model\":\"gpt-test\",\"prompt_version\":\"DG-AI-1.0\"}"}"""));
        await Assert.ThrowsAsync<InvalidOperationException>(() => CreateAnalyst(handler).AnalyzeAsync(CreateRequest()));
    }

    [Fact]
    public async Task AnalyzeAsync_ThrowsWhenApiFails()
    {
        var handler = new StubHandler(_ => Response(HttpStatusCode.BadRequest, """{"error":"bad request"}"""));
        await Assert.ThrowsAsync<HttpRequestException>(() => CreateAnalyst(handler).AnalyzeAsync(CreateRequest()));
    }

    private static AiAnalyst CreateAnalyst(StubHandler handler, int maxRetries = 0) =>
        new(new HttpClient(handler), new AiOptions { ApiKey = "test-key", Model = "gpt-test", MaxRetries = maxRetries, RetryDelayMs = 0 });
    ...
    private static AiAnalysisRequest CreateRequest()
    {
        ...
        var quant = new QuantAnalysisResult(score, new DividendQualityMetrics(null, null, null, null, null, 0, 0),
            90m, 3m, 2m, 10m, 11m, 12m, 10m, 10m,
            new FairValueRange(100m, 110m, 120m), 0.10m, zone, Array.Empty<string>(), "PARTIAL");
